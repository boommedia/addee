  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(11,22,40,0.6)', borderColor: 'rgba(0,102,255,0.3)', border: '1px solid' }}>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        {/* Add form */}
        {expanded && (
          <div className="rounded-xl p-4 space-y-3" style={{ background: 'rgba(11,22,40,0.8)', borderColor: 'rgba(0,102,255,0.3)', border: '1px solid' }}>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#7a90b8' }}>Webhook URL</label>
              <input
                value={addingUrl}
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#7a90b8' }}>Label (optional)</label>
              <input
                value={addingLabel}
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#7a90b8' }}>Events</label>
              <div className="flex flex-col gap-2">
                {EVENTS.map(ev => (
                  <button
                    ...
                    style={{
                      background: addingEvents.includes(ev.id) ? 'rgba(124,58,202,0.1)' : 'rgba(11,22,40,0.6)',
                      borderColor: addingEvents.includes(ev.id) ? 'rgba(124,58,202,0.3)' : 'rgba(0,102,255,0.3)',
                      color: addingEvents.includes(ev.id) ? '#dde4f0' : '#7a90b8',
                    }}
                  >
                    ...
                    style={{
                      borderColor: addingEvents.includes(ev.id) ? undefined : 'rgba(0,102,255,0.3)',
                    }}>
                    ...
                    <div>
                      ...
                      <div className="text-xs" style={{ color: addingEvents.includes(ev.id) ? '#7a90b8' : '#7a90b8' }}>{ev.desc}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setExpanded(false)} className="text-xs px-3 py-2 transition-colors" style={{ color: '#7a90b8' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#7a90b8' }}>Active Webhooks ({hooks.length})</p>
            {hooks.map(hook => (
              <div key={hook.id} className="flex items-start gap-3 rounded-xl px-4 py-3" style={{ background: 'rgba(11,22,40,0.6)', borderColor: 'rgba(0,102,255,0.3)', border: '1px solid' }}>
                <div className="flex-1 min-w-0">
                  {hook.label && <p className="text-xs font-semibold mb-0.5" style={{ color: '#dde4f0' }}>{hook.label}</p>}
                  <p className="text-xs truncate font-mono" style={{ color: '#7a90b8' }}>{hook.url}</p>
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    {(hook.events as string[]).map(e => (
                      <span key={e} className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ background: 'rgba(11,22,40,0.6)', borderColor: 'rgba(0,102,255,0.3)', border: '1px solid', color: '#dde4f0' }}>{e}</span>
                    ))}
                ...
                <button
                  ...
                  className="transition-colors shrink-0 mt-0.5" style={{ color: '#7a90b8' }}
                >
          ...
          <p className="text-sm text-center py-4" style={{ color: '#7a90b8' }}>No webhooks configured yet.</p>
        ) : null}
      </div>
    </div>
  )
