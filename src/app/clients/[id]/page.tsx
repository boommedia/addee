        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center justify-end group relative" title={`${d.date}: ${d.value}`}>
              <div className="w-full bg-violet-500/60 hover:bg-violet-500 rounded-sm transition-all" style={{ height: `${Math.max(pct, 2)}%` }} />
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#e8e8f0]">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <a href="/clients" className="flex items-center gap-1.5 text-[#8888a8] hover:text-white text-sm transition-colors">
        ...
        {/* Client identity */}
        <div className="bg-[#12121a] border border-[#2a2a3d] rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-4">
            {client.logo_url ? (
              <img src={client.logo_url} alt={client.name} className="w-14 h-14 rounded-xl object-contain bg-white/5 border border-[#2a2a3d] p-1 shrink-0" />
            ) : (
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                {client.industry && <span className="text-[#8888a8] text-sm">{client.industry}</span>}
                {client.website && (
                  <a href={client.website} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 text-[#555570] hover:text-cyan-400 text-xs transition-colors">
        ...
        {/* Tabs */}
        <div className="flex gap-1 border-b border-[#2a2a3d] mb-6">
          {TABS.map(t => (
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: 'Total Posts', value: posts?.length ?? 0, sub: `${postsThisMonth.length} this month`, color: 'text-violet-400' },
            ...
            <div className="bg-[#12121a] border border-[#2a2a3d] rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-[#2a2a3d] flex items-center justify-between">
                <h2 className="text-white font-bold text-sm flex items-center gap-2">
                  <FileText className="w-4 h-4 text-violet-400" /> Recent Posts
                </h2>
                {(posts?.length ?? 0) > 5 && (
                  <a href={`/clients/${id}?tab=posts`} className="text-violet-400 hover:text-violet-300 text-xs transition-colors">View all →</a>
                )}
              </div>
              {!(posts?.length) ? (
                <div className="py-10 text-center">
                  <p className="text-[#555570] text-sm">No posts yet</p>
                  <a href={`/dashboard?client=${id}`} className="inline-block mt-3 text-xs text-white bg-violet-600 hover:bg-violet-500 px-4 py-1.5 rounded-lg transition-colors">
                    Generate first post
                  </a>
                </div>
              ) : (
                  ...
                  {(posts ?? []).slice(0, 5).map(p => (
                    <div key={p.id} className="px-5 py-3 flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        <a href={`/posts/${p.id}`} className="text-white hover:text-violet-300 text-sm font-medium truncate block transition-colors">
                          {p.title ?? 'Untitled'}
                        </a>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[#555570] text-xs">{(p.word_count ?? 0).toLocaleString()} words</span>
                          {p.tone && <span className="text-[#555570] text-xs capitalize">{p.tone}</span>}
                        </div>
                      </div>
                      <div className="shrink-0 flex items-center gap-2">
                        {client.contact_email && (
                          p.wp_status === 'publish' ? (
                            <span className="text-xs text-[#555570] bg-[#1a1a26] border border-[#2a2a3d] px-2 py-0.5 rounded-full">Published</span>
                            ...
                            <form action={sendApprovalForPost}>
                              ...
                              <button type="submit" className="flex items-center gap-1 text-xs font-semibold text-white bg-violet-600 hover:bg-violet-500 px-2.5 py-1 rounded-lg transition-colors whitespace-nowrap">
                                Send Approval
                              </button>
                            </form>
                          )
                        ...
                        {p.wp_post_url ? (
                          ...
                          <span className="text-xs text-[#555570] bg-[#1a1a26] border border-[#2a2a3d] px-2 py-0.5 rounded-full">Local</span>
                        )}
            ...
            {(client.brand_voice || client.target_keywords || client.contact_email) && (
              <div className="bg-[#12121a] border border-[#2a2a3d] rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-[#8888a8] text-xs font-semibold uppercase tracking-wider">Client Info</h2>
                  <a href={`/clients/${id}?tab=settings`} className="text-violet-400 hover:text-violet-300 text-xs transition-colors">Edit →</a>
                </div>
                <div className="space-y-3">
                  {client.contact_email && (
                    <div>
                      <span className="text-[#555570] text-xs uppercase tracking-wide">Contact</span>
                      <p className="text-[#c8c8d8] text-sm mt-0.5">{client.contact_email}</p>
                    </div>
                  )}
                  {client.brand_voice && (
                    <div>
                      <span className="text-[#555570] text-xs uppercase tracking-wide">Brand Voice</span>
                      <p className="text-[#c8c8d8] text-sm mt-0.5 leading-relaxed line-clamp-3">{client.brand_voice}</p>
                    </div>
                  )}
                  {client.target_keywords && (
                    <div>
                      <span className="text-[#555570] text-xs uppercase tracking-wide">Target Keywords</span>
                      <p className="text-[#c8c8d8] text-sm mt-0.5">{client.target_keywords}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-[#12121a] border border-[#2a2a3d] rounded-2xl overflow-hidden">
            {!(posts?.length) ? (
              <div className="py-20 text-center">
                <FileText className="w-8 h-8 text-[#2a2a3d] mx-auto mb-3" />
                <p className="text-[#8888a8] text-sm">No posts yet</p>
                <a href={`/dashboard?client=${id}`} className="inline-block mt-4 text-xs text-white bg-violet-600 hover:bg-violet-500 px-4 py-2 rounded-lg transition-colors">
                  Generate first post
                </a>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#2a2a3d]">
                      <th className="text-left text-[#8888a8] text-xs font-semibold uppercase tracking-wider px-5 py-3">Title</th>
                      <th className="text-left text-[#8888a8] text-xs font-semibold uppercase tracking-wider px-4 py-3 hidden md:table-cell">Tone</th>
                      <th className="text-left text-[#8888a8] text-xs font-semibold uppercase tracking-wider px-4 py-3 hidden sm:table-cell">Words</th>
                      <th className="text-left text-[#8888a8] text-xs font-semibold uppercase tracking-wider px-4 py-3">Status</th>
                      <th className="text-left text-[#8888a8] text-xs font-semibold uppercase tracking-wider px-4 py-3 hidden lg:table-cell">Date</th>
                      <th className="px-4 py-3" />
                    </tr>
                  </thead>
                  <tbody>
                    {(posts ?? []).map((p, i) => (
                      <tr key={p.id} className={`${i % 2 === 0 ? '' : 'bg-[#0d0d14]'} hover:bg-[#1a1a26]/50 transition-colors`}>
                        <td className="px-5 py-3">
                          <a href={`/posts/${p.id}`} className="text-white hover:text-violet-300 text-sm font-medium line-clamp-1 transition-colors">
                            {p.title ?? 'Untitled'}
                          </a>
                        </td>
                        <td className="px-4 py-3 hidden md:table-cell">
                          {p.tone && <span className="text-xs capitalize text-[#8888a8] bg-[#1a1a26] border border-[#2a2a3d] px-2 py-0.5 rounded-full">{p.tone}</span>}
                        </td>
                        <td className="px-4 py-3 text-[#8888a8] text-sm hidden sm:table-cell">{(p.word_count ?? 0).toLocaleString()}</td>
