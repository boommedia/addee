...
function ProfileDrawer({ user, onClose, onDone }: {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-[440px] bg-[#0d0d14] border-l border-[#2a2a3d] z-50 flex flex-col shadow-2xl">

        {/* Header */}
        <div className="flex items-start gap-3 px-5 py-4 border-b border-[#2a2a3d] shrink-0">
          <div className="w-11 h-11 rounded-full bg-violet-500/20 border border-violet-500/30 flex items-center justify-center text-violet-300 text-lg font-black shrink-0">
            {user.email.charAt(0).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            {user.full_name && <div className="text-white font-bold text-sm leading-tight">{user.full_name}</div>}
            {user.agency_name && <div className="text-[#8888a8] text-xs">{user.agency_name}</div>}
            <div className="text-[#555570] text-xs truncate">{user.email}</div>
            <div className="text-[#3a3a5a] text-xs font-mono mt-0.5">{user.id.slice(0, 12)}…</div>
          </div>
          <button onClick={onClose} className="text-[#555570] hover:text-white p-1.5 rounded-lg hover:bg-[#2a2a3d] transition-colors shrink-0 mt-0.5">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex border-b border-[#2a2a3d] shrink-0">
          {(['profile', 'message'] as const).map(t => (
            <button
              ...
              className={`flex items-center gap-1.5 px-5 py-2.5 text-xs font-semibold capitalize transition-colors ${
                tab === t ? 'text-white border-b-2 border-violet-500' : 'text-[#8888a8] hover:text-white'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === 'profile' && (
            <div className="p-5 space-y-5">
              ...
              {/* Plan + account info */}
              <div className="bg-[#12121a] border border-[#2a2a3d] rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {!editingPlan && (
                    <button
                      ...
                      className="text-xs text-[#8888a8] hover:text-white bg-[#1a1a26] hover:bg-[#2a2a3d] border border-[#2a2a3d] px-2.5 py-1 rounded-lg transition-colors flex items-center gap-1"
                    >
                ...
                {editingPlan && (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setEditingPlan(false)} className="p-1.5 rounded-lg hover:bg-[#2a2a3d] text-[#8888a8] transition-colors">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-xs pt-1">
                  {[
                    { label: 'Status', value: user.plan_status ?? 'none', color: isActive ? 'text-emerald-400' : 'text-[#555570]' },
                    ...
                    <div key={item.label}>
                      <div className="text-[#555570] mb-0.5">{item.label}</div>
              ...
              {/* Usage stats */}
              <div>
                <div className="text-[#8888a8] text-xs font-semibold uppercase tracking-wider mb-3">Usage</div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={s.label} className="bg-[#12121a] border border-[#2a2a3d] rounded-xl p-3">
                      <div className="text-[#555570] text-xs mb-1">{s.label}</div>
                      <div className={`font-black text-xl leading-none mb-0.5 ${s.color}`}>{s.value}</div>
                      <div className="text-[#555570] text-xs">{s.sub}</div>
                    </div>
                  ))}
                </div>
                {usagePct > 0 && (
                  <div className="mt-3">
                    <div className="w-full h-1.5 bg-[#2a2a3d] rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${usagePct >= 90 ? 'bg-red-500' : usagePct >= 70 ? 'bg-yellow-500' : 'bg-violet-500'}`}
                        style={{ width: `${usagePct}%` }}
                      />
                    </div>
                    <div className="text-[#3a3a5a] text-xs mt-1 text-right">{user.posts_this_month}/{user.posts_limit} posts this month</div>
                  </div>
                )}
              </div>
              ...
              {/* Actions */}
              <div>
                <div className="text-[#8888a8] text-xs font-semibold uppercase tracking-wider mb-3">Actions</div>
                <div className="flex flex-col gap-2">
                  {[
                    { action: 'reset-password', icon: <KeyRound className="w-3.5 h-3.5" />, label: 'Send Password Reset Email', cls: 'text-white hover:bg-[#2a2a3d]' },
                    user.banned
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm bg-[#12121a] border border-[#2a2a3d] ${item.cls} transition-colors disabled:opacity-50`}
                    >
            ...
            <div className="p-5 space-y-4">
              {sent ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <div className="text-[#555570] text-xs mt-1">Delivered to {user.email}</div>
                </div>
              ) : (
                <>
                  {/* To */}
                  <div>
                    <div className="text-[#555570] text-xs mb-1.5">To</div>
                    <div className="bg-[#12121a] border border-[#2a2a3d] rounded-xl px-3 py-2.5 text-[#8888a8] text-sm flex items-center gap-2">
                  ...
                  {/* Quick templates */}
                  <div>
                    <div className="text-[#555570] text-xs mb-2">Quick templates</div>
                        ...
                        <button
                          ...
                          className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${
                            template === t.id
                              ...
                              : 'bg-[#12121a] border-[#2a2a3d] text-[#8888a8] hover:text-white hover:border-[#3a3a5a]'
                          }`}
                        >
                          {t.label}
                        </button>
                  ...
                  {/* Subject */}
                  <div>
                    <div className="text-[#555570] text-xs mb-1.5">Subject</div>
                    <input
                      type="text"
                      value={subject}
                      onChange={e => setSubject(e.target.value)}
                      placeholder="Email subject…"
                      className="w-full bg-[#12121a] border border-[#2a2a3d] focus:border-violet-500 rounded-xl px-3 py-2.5 text-white text-sm placeholder-[#3a3a5a] outline-none transition-colors"
                    />
                  </div>

                  {/* Body */}
                  <div>
                    <div className="text-[#555570] text-xs mb-1.5">Message</div>
                    <textarea
                      value={body}
                      onChange={e => setBody(e.target.value)}
                      rows={11}
                      placeholder="Write your message…"
                      className="w-full bg-[#12121a] border border-[#2a2a3d] focus:border-violet-500 rounded-xl px-3 py-2.5 text-white text-sm placeholder-[#3a3a5a] outline-none resize-none transition-colors leading-relaxed"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white font-semibold text-sm py-3 rounded-xl transition-colors"
                  >
                    {sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                    {sending ? 'Sending…' : 'Send Email'}
                  </button>

                  <p className="text-[#3a3a5a] text-xs text-center">Sent from eric@bloggy.online via Resend</p>
                </>
              )}
            </div>
          )}
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#555570]" />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search by email, name, agency…"
            className="w-full bg-[#12121a] border border-[#2a2a3d] rounded-lg pl-9 pr-3 py-2 text-white text-sm placeholder-[#555570] focus:outline-none focus:border-violet-500 transition-colors"
          />
        </div>
        <select
          value={planFilter}
          onChange={e => setPlanFilter(e.target.value)}
          className="bg-[#12121a] border border-[#2a2a3d] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-violet-500 transition-colors"
        >
        ...
        <span className="text-[#555570] text-xs">{filtered.length} user{filtered.length !== 1 ? 's' : ''}</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2a2a3d]">
                <th className="text-left px-5 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider">User</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider">Plan</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider">Posts / mo</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider hidden sm:table-cell">Total</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider hidden md:table-cell">Words</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider hidden lg:table-cell">Sites</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider hidden lg:table-cell">Last Active</th>
                <th className="text-left px-4 py-3 text-[#8888a8] text-xs font-semibold uppercase tracking-wider hidden xl:table-cell">Joined</th>
            ...
            <tbody>
              {filtered.map((user, i) => {
                ...
                return (
                  <tr
                    key={user.id}
                    className={clsx(
                      'border-b border-[#1a1a26] hover:bg-[#1a1a26] transition-colors cursor-pointer',
                      i % 2 === 0 ? '' : 'bg-[#0d0d14]',
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-14 h-1.5 bg-[#2a2a3d] rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${usagePct >= 90 ? 'bg-red-500' : usagePct >= 70 ? 'bg-yellow-500' : 'bg-violet-500'}`}
                            style={{ width: `${usagePct}%` }}
                          />
                        </div>
                    ...
                    <td className="px-4 py-3 text-[#8888a8] text-xs hidden lg:table-cell">
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => setSelectedUser(user)}
                        className="text-xs text-[#555570] hover:text-violet-400 bg-[#1a1a26] hover:bg-[#2a2a3d] border border-[#2a2a3d] px-2.5 py-1 rounded-lg transition-colors whitespace-nowrap"
                      >
                        View →
                      </button>
                    </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="px-5 py-12 text-center text-[#555570] text-sm">No users match your filters</td></tr>
              )}
            </tbody>
          </table>
        </div>
